<?php	
	...
	//conexion
	$idcon = fncconn();
	$rs_equipo = dinamicscanopequipo(array('sistemcodigo' => 64, 'tipequcodigo' => 4),array('sistemcodigo' => '=', 'tipequcodigo' => '='),$idcon);
...
<html>
	<head>
		<title>Programa de pauchado</title>
		...
		<script type="text/javascript" src="../src/FunjQuery/jquery.jsscripts/jquery.function_bandejapch.js"></script>
		<script type="text/javascript">
			$(function(){
				$( "<?php echo $sortable ?>" ).sortable().disableSelection();

		        var $tabs = $( "#programapauchado" ).tabs();
		...
		<div id="programapauchado">
			...
			<?php 
				...
				for($a = 0;$a < $nr_equipo;$a++)
				{
					...
					$rsPauchado = dinamicscanopprogramapauchado1(array('equipocodigo' => $obj_equipo),array('equipocodigo' => '='),$idcon);
					//se consulta el numero de resgirstro de la consulta
					$nrPauchado = fncnumreg($rsPauchado);
			?>
			<script type="text/javascript">
				Event_animatedcollapse('<?php echo $nrPauchado ?>', 'filtrPropch_<?php echo $obj_equipo ;?>');
			</script>
				...
				<ul id="<?php echo $obj_sortable ?>" class="connectedSortable ui-helper-reset">
					<?php 
						...
						for($b = 0;$b < $nrPauchado;$b++)
						{
							...
							$checked = '';
							if($rwPauchado['ordoppcomfir'] > 0)
							{
								$checked = 'checked';
								$arroppview = ($arroppview)? $arroppview.','.$rwPauchado['ordoppcodigo'] : $rwPauchado['ordoppcodigo'];
							}
					...
					?>
					<li class="ui-state-default"  id="<?php echo $rwPauchado['ordoppcodigo'] ?>" style="width:100%;">
						<table width="100%" border="0" cellspacing="1" cellpadding="1" align="center">
							<tr <?php echo $complement ?>">
								<td width="6%" class="cont-line">&nbsp;<?php echo str_pad($rwPauchado['ordoppcodigo'], 4, "0", STR_PAD_LEFT) ?></td>
								...
								<td width="6%" class="cont-line">&nbsp;<?php echo number_format($KILOSPAUCHADO, 2, ',', '.'); ?></td>
								<td width="5%" class="cont-line">&nbsp;<input type="checkbox" id="chkoppview" name="chkoppview" <?php echo $checked ?> onclick="setSelectionRow(this.value, document.getElementById('arroppview').value, ',', 'arroppview');" value="<?php echo $rwPauchado['ordoppcodigo'] ?>"></td>
							</tr>
						</table>
					</li>
					<?php 
		...
		<table width="100%" align="center" cellpadding="1" cellspacing="1">
			...
			<tr>
				<td class="NoiseErrorDataTD" align="center"><div class="ui-buttonset">
					<button id="editaprograma_pch">Editar [programa]</button>
					<button id="cancelarprograma_pch">Atras</button></div>
				</td>
			</tr>
			<tr><td>&nbsp;</td></tr> 
		</table>
