...
function editaanalisismp($iReganalisismp,&$flageditaranalisismp,&$campnomb,&$codigo,$flagerror)
{
	$nuconn = fncconn();
	define("errorReg",1);
	define("errorCar",2);
	...
	{ 
		...
		$iReganalisismp_b = $iReganalisismp;
		...
		if($flagerror != 1)
		{
			$result = uprecordanalisismp($iReganalisismp,$nuconn);
			if($result < 0 )
			{
				ob_end_clean();
...
editaanalisismp($iReganalisismp,$flageditaranalisismp,$campnomb,$codigo,$flagerror);
