...
Funcion         : editausuagrupcapa
Decripcion      : Valida la data a grabar y la lleva al paquete.
Parametros      : Descripicion
$iRegusuagrupcapa         Arreglo de datos.
$flageditarusuagrupcapa   Bandera de validación
...
| Fecha 		| Motivo													| Autor 	|
13-jul-2005	 Acondicionarlo a los nuevos requerimientos de producción	 jcortes		*/
...
function grabausuagrupcapa($iRegusuagrupcapa,&$flageditarusuagrupcapa,&$campnomb,&$bandera)
{
	$nuconn = fncconn();
		...
		if($flagerror != 1)
		{
			...
			if($result > 0)
			{
				...
				if ($bandera == 1)
				{
					...
					echo '<!--//'."\n";
					echo 'location ="maestablusuagrupcapa.php?codigo='.$codigo.';"';
					echo '//-->'."\n";
					echo '</script>';
				}
			}
...
*/
function validacupo($grupo,$colab,$contcolb,$nucon,&$accioneditaremplgrupo)
{
...
if($grucapcodigo)
{
	if($arreglo1)
	{
		...
		else
		{
			...
			{
				$idval = validacupo($grucapcodigo,$valposic[$i],$numposic,$idcon,$accioneditaremplgrupo);
				if($idval ==1)
				{
					...
					$x++;
					grabausuagrupcapa($iRegusuagrupcapa,$flageditarusuagrupcapa,$campnomb,$x);
					$accioneditaremplgrupo = 0;
				}
			}
