	...
	if($iRegopp)
	{
		...
		if($flagerror != 1)
		{
			...
			if($result > 0)
			{
				...
				fncmsgerror(grabaEx);	
				insrecordprogramalaminado(array('ordoppcodigo' => $iRegopp['ordoppcodigo'], 'prograindice' => 999),$nuconn);			
			}
			fncclose($nuconn);
		}
	}
...
if(!$flagnuevoopp)
{
	...
	//-----------------------------------------------------------
	$gen_tiposoliprog = 2;//solicitudes de laminado
	//include('genopp.php');
	//se envia al maestro
	echo '<script language="javascript">';
	echo '<!--//'."\n";
	echo 'location ="maestablbandejalaminado.php?codigo='.$codigo.';"';
	echo '//-->'."\n";
	echo '</script>';
}
?> 
