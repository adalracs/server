...
function editanocomformepr($iRegnocomformepr,&$flageditarnocomformepr,&$campnomb,&$codigo)
{
	$nuconn = fncconn();
	define("errorReg",1);
	define("errorCar",2);
	...
	{ 
		...
		$iRegnocomformepr_b = $iRegnocomformepr;

		while($elementos = each($iRegnocomformepr))
		{ 
		...
		if($flagerror != 1)
		{
			$result = uprecordnoconformepr($iRegnocomformepr,$nuconn);
			if($result < 0 )
			{
				ob_end_clean();
			...
			if($result > 0)
			{
				...
				echo '<!--//'."\n";
				echo 'location ="maestablnoconformepr.php?codigo='.$codigo.';"';
				echo '//-->'."\n";
				echo '</script>';
			}
			fncclose($nuconn);
...
editanocomformepr($iRegnoconformepr,$flageditarnocomformepr,$campnomb,$codigo);
