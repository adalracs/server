...
function grabatipoequipo($iRegtipoequipo, &$flagnuevotipoequipo, &$campnomb, &$tipequcod)
{
	$nuconn = fncconn();
	...
	do
	{
		$nuresult = loadrecordtipoequipo($nuidtemp,$nuconn);
		if($nuresult == e_empty)
		{
			$iRegtipoequipo[tipequcodigo] = $nuidtemp;
			$tipequcod = $iRegtipoequipo[tipequcodigo];
		}
		$nuidtemp ++;
...
/*
			if(($elementos[0] == 'tipequnombre') || ($elementos[0] == 'tipequacroni'))
			{
				if($elementos[1] != null)
				{
					$validnombre =  fncnombexs('tipoequipo', $iRegtipoequipo, $elementos[0], $elementos[1], $nuconn);
		...
		if($flagerror != 1)
		{
			$result = insrecordtipoequipo($iRegtipoequipo,$nuconn);
			if($result < 0 )
			{
				ob_end_clean();
...
grabatipoequipo($iRegtipoequipo,$flagnuevotipoequipo,$campnomb,$tipequcod);

if(!$flagnuevotipoequipo)
{
	if($arreglo_aux)
	{
		include('grabatipoequipocamperequipo.php');
		unset($arreglo_aux);
	}
}
?>