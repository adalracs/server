<?php 
	...
	if($accionnuevovistadesarrollo) 
		include ( 'grabavistadesarrollo.php');
...
<?php if (! $codigo) echo "<!--"; ?> 
	<body bgcolor="FFFFFF" text="#000000"> 
		<form name="form1" method="post" enctype="multipart/form-data"> 
			<p><font class="NoiseFormHeaderFont">Pedido/&Iacute;tem [Desarrollo]</font></p> 
...
<?php endif; ?>
				...
				<tr>
					<td>
						...
						<div id="tabitems">
							...
							<?php if($tipitecodigo == 1 && ($tipevecodigo != 3)) include '../src/FunjQuery/jquery.tabs/desarrollo/jquery.bolsaflowpack.php' ?>
							<?php if($tipitecodigo == 2 && ($tipevecodigo != 3)) include '../src/FunjQuery/jquery.tabs/desarrollo/jquery.bolsalateral.php' ?>
							<?php if($tipitecodigo == 3 && ($tipevecodigo != 3)) include '../src/FunjQuery/jquery.tabs/desarrollo/jquery.bolsapouchdoypack.php' ?>
							<?php if($tipitecodigo == 4 && ($tipevecodigo != 3)) include '../src/FunjQuery/jquery.tabs/desarrollo/jquery.bolsapouchlateral.php' ?>
							<?php if($tipitecodigo == 5 && ($tipevecodigo != 3)) include '../src/FunjQuery/jquery.tabs/desarrollo/jquery.capuchon.php' ?>
							<?php if($tipitecodigo == 6 && ($tipevecodigo != 3)) include '../src/FunjQuery/jquery.tabs/desarrollo/jquery.lamina.php' ?>
						</div>
						<?php endif?>
					</td>
				</tr>
			...
			<input type="hidden" name="flagnuevovistadesarrollo">

			<input type="hidden" name="modulocodigo" id="modulocodigo" value="3"><!-- modulo de desarrollo -->
			...
			<input type="hidden" name="pedvencodigo" value="<?php if(!$flagnuevovistadesarrollo){ echo $rwPedidoventa[pedvencodigo];}else{ echo $pedvencodigo; }?>"> 
			<input type="hidden" name="produccodigo" id="produccodigo" value="<?php echo $produccodigo; ?>">
			<input type="hidden" name="ordcomcodigo" value="<?php if(!$flagnuevovistadesarrollo){ echo $rwOrdencompra[ordcomcodigo];}else{ echo $ordcomcodigo; } ?>">  		
			<input type="hidden" name="propedcodigo" value="<?php if(!$flagnuevovistadesarrollo){ echo $rwProducpedido[propedcodigo];}else{ echo $propedcodigo; } ?>">  		
			...
			<input type="hidden" name="accionnuevovistadesarrollo"> 
