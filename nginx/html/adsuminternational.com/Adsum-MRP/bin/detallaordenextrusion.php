...
ini_set('display_errors',1);
	...
	if($plantacodigo || $formulnumero || $ordprocalibr || $ordproancext || $paditecodigo)
	{
		...
		$record['ordoppanchot'] = $ordproancext;
		$recordop['ordoppanchot'] = '=';
		$record['paditecodigo'] = $paditecodigo;
		$recordop['paditecodigo'] = '=';
		$rsExtrusion = dinamicscanopprogramaextrusion1($record, $recordop, $idcon);
	}
	else
	{
		$rsExtrusion = fullscanprogramaextrusion1($idcon);
	}
...
<script type="text/javascript">
	Event_animatedcollapse('<?php echo $nrExtrusion ?>', 'filtrOpp');
</script>
...
<?php 
		for($b = 0; $b < $nrExtrusion; $b++)
		{
			...
			$totalopp_kgs = $totalopp_kgs + $rwExtrusion['ordoppcantkg'];
			$totalopp_mts = $totalopp_mts + $rwExtrusion['ordoppcantmt'];
			...
			$SPANEXTRUSION = ($rwExtrusion['equipocodigo'])? 'ui-icon ui-icon-check' : 'ui-icon ui-icon-help' ;
...
?>		
	<tr <?php echo $complement ?>">
		...
		<td width="5%" class="cont-line"><input type="radio" id="chkopp" name="chkopp" <?php echo $checked; ?> onclick="setArropp(this.value);" value="<?php echo $rwExtrusion['ordoppcodigo'] ?>"></td>
		<td width="5%" class="cont-line">&nbsp;<font color="blue"><b><?php echo str_pad($rwExtrusion['ordoppcodigo'], 4, "0", STR_PAD_LEFT); ?></b></font></td>
