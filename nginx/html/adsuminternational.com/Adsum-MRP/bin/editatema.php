...
function editatema(&$iRegtema,&$flageditartema,&$campnomb,&$codigo)
{
	$nuconn = fncconn();
	define("errorReg",1);
	define("errorCar",2);
	...
	{ 
		...
		if($flagerror != 1)
		{
			$result = uprecordtema($iRegtema,$nuconn);
			if($result < 0 )
			{
				ob_end_clean();
			...
			if($result > 0)
			{
				...
				echo '<!--//'."\n";
				echo 'location ="maestabltema.php?codigo='.$codigo.';"';
				echo '//-->'."\n";
				echo '</script>';
			}
			fncclose($nuconn);
...
editatema($iRegtema,$flageditartema,$campnomb,$codigo);