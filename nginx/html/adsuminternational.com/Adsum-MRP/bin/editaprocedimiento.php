...
function editaprocedimiento($iRegprocedimiento,&$flageditarprocedimiento,&$campnomb,&$codigo)
{
	$nuconn = fncconn();
	define("errorReg",1);
	define("errorCar",2);
	...
	{ 
		...
		if($flagerror != 1)
		{
			$result = uprecordprocedimiento($iRegprocedimiento,$nuconn);
			if($result < 0 )
			{
				ob_end_clean();
...
editaprocedimiento($iRegprocedimiento,$flageditarprocedimiento,$campnomb,$codigo);
?> 
