...
function editatipofall($iRegtipofall,&$flageditartipofall,&$campnomb,&$codigo) 
{ 
	$nuconn = fncconn(); 
	define("errorReg",1); 
	define("errorCar",2); 
	...
	{ 
		...
		{ 
			...
			if($elementos[0]=='tipfalnombre')
			{
				if($elementos[1] != null)
				{
					$validnombre =  fncnombeditexs('tipofall',$iRegtipofall,$elementos[0],$elementos[1],'tipfalcodigo',$iRegtipofall[tipfalcodigo],$nuconn);
					if ($validnombre == 1)
					{
		...
		if($flagerror != 1) 
		{ 
			$result = uprecordtipofall($iRegtipofall,$nuconn); 
			if($result < 0 ) 
			{ 
				ob_end_clean(); 
			...
			{ 
				...
				echo '<!--//'."\n"; 
				echo 'location ="maestabltipofall.php?codigo='.$codigo.';"'; 
				echo '//-->'."\n"; 
				echo '</script>'; 
			} 
			fncclose($nuconn); 
...
editatipofall($iRegtipofall,$flageditartipofall,$campnomb,$codigo); 
?> 
