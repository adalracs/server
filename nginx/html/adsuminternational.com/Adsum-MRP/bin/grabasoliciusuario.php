...
Fecha: 20120110 
GenVers: 4.8 --> 
Funcion         : grabasoliciusuario 
Decripcion      : Valida la data a grabar y la lleva al paquete. 
Parametros      : Descripicion 
    $iRegsoliciusuario         Arreglo de datos. 
    $flagnuevosoliciusuario    Bandera de validación 
Retorno         : 
		true	= 1 
		false	= 0 
Autor           : ariascos 
...
function grabasoliciusuario($iRegsoliciusuario,&$flagnuevosoliciusuario,&$campnomb) 
{ 
	$nuconn = fncconn(); 
	...
	do 
	{ 
		$nuresult = loadrecordsoliciusuario($nuidtemp,$nuconn); 
		if($nuresult == e_empty) 
		{ 
			$iRegsoliciusuario[tisouscodigo] = $nuidtemp; 
		} 
		$nuidtemp ++; 
	...
	{ 
		...
		if($flagerror != 1) 
		{ 
			$result = insrecordsoliciusuario($iRegsoliciusuario,$nuconn); 
			if($result < 0 ) 
			{ 
				ob_end_clean(); 
...
grabasoliciusuario($iRegsoliciusuario,$flagnuevosoliciusuario,$campnomb); 
?> 
