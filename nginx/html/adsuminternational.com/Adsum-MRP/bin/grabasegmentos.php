...
function grabasegmentos($iRegsegmentos,&$flagnuevosegmentos,&$campnomb){ 
	$nuconn = fncconn(); 
	...
	do{ 
		$nuresult = loadrecordsegmentos($nuidtemp,$nuconn); 
		if($nuresult == e_empty){ 
			$iRegsegmentos[segmencodigo] = $nuidtemp; 
		} 
		$nuidtemp ++; 
	...
	if ($iRegsegmentos){ 
		...
		while($elementos = each($iRegsegmentos)){
			...
			if($elementos[0]=='segmennombre'){
				if($elementos[1] != null){
					$validnombre =  fncnombexs('segmentos',$iRegsegmentos,$elementos[0],$elementos[1],$nuconn);
		...
		if($flagerror != 1){ 
			$result = insrecordsegmentos($iRegsegmentos,$nuconn); 
...
grabasegmentos($iRegsegmentos,$flagnuevosegmentos,$campnomb); 
?> 