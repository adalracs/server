<?php	
	...
	//conexion
	$idcon = fncconn();
	$rs_equipo = dinamicscanopequipo(array('sistemcodigo' => 66, 'tipequcodigo' => 4),array('sistemcodigo' => '=', 'tipequcodigo' => '='),$idcon);
...
<html>
	<head>
		<title>Programa de valvulado</title>
		...
		<script type="text/javascript" src="../src/FunjQuery/jquery.jsscripts/jquery.function_bandejavlv.js"></script>
		<script type="text/javascript">
			$(function(){
				$( "<?php echo $sortable ?>" ).sortable().disableSelection();

		        var $tabs = $( "#programavalvulado" ).tabs();
		...
		<div id="programavalvulado">
			...
			<?php 
				...
				for($a = 0;$a < $nr_equipo;$a++)
				{
					...
					$rsValvulado = dinamicscanopprogramavalvulado1(array('equipocodigo' => $obj_equipo),array('equipocodigo' => '='),$idcon);
					//se consulta el numero de resgirstro de la consulta
					$nrValvulado = fncnumreg($rsValvulado);
			?>
			<script type="text/javascript">
				Event_animatedcollapse('<?php echo $nrValvulado ?>', 'filtrProvlv_<?php echo $obj_equipo ;?>');
			</script>
				...
				<ul id="<?php echo $obj_sortable ?>" class="connectedSortable ui-helper-reset">
					<?php 
						...
						for($b = 0;$b < $nrValvulado;$b++)
						{
							...
							$checked = '';
							if($rwValvulado['ordoppcomfir'] > 0)
							{
								$checked = 'checked';
								$arroppview = ($arroppview)? $arroppview.','.$rwValvulado['ordoppcodigo'] : $rwValvulado['ordoppcodigo'];
							}
					...
					?>
					<li class="ui-state-default"  id="<?php echo $rwValvulado['ordoppcodigo'] ?>" style="width:100%;">
						<table width="100%" border="0" cellspacing="1" cellpadding="1" align="center">
							<tr <?php echo $complement ?>">
								<td width="6%" class="cont-line">&nbsp;<?php echo str_pad($rwValvulado['ordoppcodigo'], 4, "0", STR_PAD_LEFT) ?></td>
								...
								<td width="6%" class="cont-line">&nbsp;<?php echo number_format($KILOSVALVULADO, 2, ',', '.'); ?></td>
								<td width="5%" class="cont-line">&nbsp;<input type="checkbox" id="chkoppview" name="chkoppview" <?php echo $checked ?> onclick="setSelectionRow(this.value, document.getElementById('arroppview').value, ',', 'arroppview');" value="<?php echo $rwValvulado['ordoppcodigo'] ?>"></td>
							</tr>
						</table>
					</li>
					<?php 
		...
		<table width="100%" align="center" cellpadding="1" cellspacing="1">
			...
			<tr>
				<td class="NoiseErrorDataTD" align="center"><div class="ui-buttonset">
					<button id="editaprograma_vlv">Editar [programa]</button>
					<button id="cancelarprograma_vlv">Atras</button></div>
				</td>
			</tr>
			<tr><td>&nbsp;</td></tr> 
		</table>
