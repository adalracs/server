...
function editacargo($iRegcargo,&$flageditarcargo,&$campnomb,&$codigo) 
{ 
	$nuconn = fncconn(); 
	define("errorReg",1); 
	define("errorCar",2); 
	...
	{ 
		...
		{ 
			...
			if($elementos[0]=='cargonombre')
			{
				if($elementos[1] != null)
				{
					$validnombre = fncnombeditexs('cargo',$iRegcargo,$elementos[0],$elementos[1],'cargocodigo',$iRegcargo['cargocodigo'],$nuconn);
					if ($validnombre == 1)
					{
		...
		if($flagerror != 1) 
		{ 
			$result = uprecordcargo($iRegcargo,$nuconn); 
			if($result < 0 ) 
			{ 
				ob_end_clean(); 
			...
			{ 
				...
				echo '<!--//'."\n"; 
				echo 'location ="maestablcargo.php?codigo='.$codigo.';"'; 
				echo '//-->'."\n"; 
				echo '</script>'; 
			} 
			fncclose($nuconn); 
...
editacargo($iRegcargo,$flageditarcargo,$campnomb,$codigo); 
?> 
