...
$flagnuevoproveedo    Bandera de validaci�n
Retorno         :
true	= 1
false	= 0
Autor           : ariascos
...
function grabaproveedo($iRegproveedo,&$flagnuevoproveedo,&$campnomb)
{
	$nuconn = fncconn();
	...
	do
	{
		...
		if($nuresult == e_empty)
		{
			$iRegproveedo[proveecodigo] = $nuidtemp;
			$idprovee=$nuidtemp;
		}
		$nuidtemp ++;
	...
	if($iRegproveedo)
	{
		...
		while($elementos = each($iRegproveedo))
		{
			...
			if($elementos[0]=='proveenombre')
			{
				if($elementos != null)
				{
					$validnombre =  fncnombexs('proveedo',$iRegproveedo,$elementos[0],$elementos[1],$nuconn);
					if ($validnombre == 1)
					{
...
//			}
		...
		if($flagerror != 1)
		{
			$result = insrecordproveedo($iRegproveedo,$nuconn);
			if($result < 0 )
			{
...
$iRegproveedo[proveeciudad] = $proveeciudad;
...
$proveecodi=grabaproveedo($iRegproveedo,$flagnuevoproveedo,$campnomb);
