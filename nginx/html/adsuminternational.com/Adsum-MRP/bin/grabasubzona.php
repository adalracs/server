...
Funcion         : grabasubzona
Decripcion      : Valida la data a grabar y la lleva al paquete. 
Parametros      : Descripicion 
    $iRegsubzona         Arreglo de datos. 
    $flagnuevosubzona    Bandera de validaci�n 
Retorno         : 
		true	= 1 
		false	= 0 
Autor           : cbedoya
...
function grabasubzona($iRegsubzona,&$flagnuevosubzona,&$campnomb){ 
	$nuconn = fncconn(); 
	...
	do{ 
		$nuresult = loadrecordsubzona($nuidtemp,$nuconn); 
		if($nuresult == e_empty){ 
			$iRegsubzona[subzoncodigo] = $nuidtemp; 
		} 
		$nuidtemp ++; 
	...
	if ($iRegsubzona){ 
		...
		while($elementos = each($iRegsubzona)){
			...
			if($elementos[0]=='subzonnombre'){
				if($elementos[1] != null){
					$validnombre =  fncnombexs('subzona',$iRegsubzona,$elementos[0],$elementos[1],$nuconn);
		...
		if($flagerror != 1){ 
			$result = insrecordsubzona($iRegsubzona,$nuconn); 
...
grabasubzona($iRegsubzona,$flagnuevosubzona,$campnomb); 
?> 
