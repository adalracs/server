...
ini_set('display_errors',1);
	...
	if($plantacodigo || $formulnumero || $ordprocalibr || $ordproancext || $paditecodigo)
	{
		...
		$record['ordproancext'] = $ordproancext;
		$recordop['ordproancext'] = '=';
		$record['paditecodigo'] = $paditecodigo;
		$recordop['paditecodigo'] = '=';
		$rsOp = dinamicscanopvistabandejaextrusion($record, $recordop, $idcon);
	}
	else
	{
		$rsOp = fullscanvistabandejaextrusion($idcon);
	}
...
?>			
	<tr <?php echo $complement ?>>
		...
		<td width="5%" class="cont-line"><input type="checkbox" id="chkop" name="chkop" <?php echo $checked ?> onclick="setSelectionRow(this.value, document.getElementById('arrop').value, ',', 'arrop');" value="<?php echo $rwOp['ordprocodigo'] ?>"></td>
