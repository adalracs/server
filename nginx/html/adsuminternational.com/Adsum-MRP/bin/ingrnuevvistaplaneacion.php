<?php
	...
	include ('../src/FunPerPriNiv/pktblplanearutaitemest.php');
	...
	if($accionnuevovistaplaneacion){ 
		include ( 'grabavistaplaneacion.php');
	}
	...
	if(!$flagnuevovistaplaneacion)
	{
		...
		$ordcomcodcli = $rwOrdencompra['ordcomcodcli'];//codigo del client		
		...
		if($tipevecodigo == 3){
			...
			unset($cant_planea, $criterio, $criterio_val);
		}
...
<?php endif; ?>
				...
				<tr>
					<td>
						...
						<div id="tabitems">
							...
							<?php if($tipitecodigo == 1) include '../src/FunjQuery/jquery.tabs/planeacion/jquery.bolsaflowpack.php' ?>
							<?php if($tipitecodigo == 2) include '../src/FunjQuery/jquery.tabs/planeacion/jquery.bolsalateral.php' ?>
							<?php if($tipitecodigo == 3) include '../src/FunjQuery/jquery.tabs/planeacion/jquery.bolsapouchdoypack.php' ?>
							<?php if($tipitecodigo == 4) include '../src/FunjQuery/jquery.tabs/planeacion/jquery.bolsapouchlateral.php' ?>
							<?php if($tipitecodigo == 5) include '../src/FunjQuery/jquery.tabs/planeacion/jquery.capuchon.php' ?>
							<?php if($tipitecodigo == 6) include '../src/FunjQuery/jquery.tabs/planeacion/jquery.lamina.php' ?>
						</div>
						<?php endif?>
					</td>
				</tr>
			...
			<input type="hidden" name="flagnuevovistaplaneacion">
			...
			<input type="hidden" name="pedvencodigo" value="<?php if(!$flagnuevovistaplaneacion){ echo $rwPedidoventa[pedvencodigo];}else{ echo $pedvencodigo; }?>"> 
			<input type="hidden" name="produccodigo" id="produccodigo" value="<?php if(!$flagnuevovistaplaneacion){ echo $sbreg[produccodigo];}else{ echo $produccodigo; } ?>">  		
			<input type="hidden" name="ordcomcodigo" value="<?php if(!$flagnuevovistaplaneacion){ echo $rwOrdencompra[ordcomcodigo];}else{ echo $ordcomcodigo; } ?>">  		
			<input type="hidden" name="propedcodigo" value="<?php if(!$flagnuevovistaplaneacion){ echo $rwProducpedido[propedcodigo];}else{ echo $propedcodigo; } ?>">  		
			...
			<input type="hidden" name="accionnuevovistaplaneacion"> 
			...
			<input type="hidden" name="traslape" id="traslape" value="<?php echo ($traslape)? $traslape : 0 ; ?>" /> 
