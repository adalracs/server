...
function editacurso($iRegcurso,&$flageditarcurso,&$campnomb,&$codigo)
{
	$nuconn = fncconn();
	define("errorReg",1);
	define("errorCar",2);
	...
	{ 
		...
		{ 
			...
			if($elementos[0]=='cursonombre')
			{
				if($elementos[1] != null)
				{
					$validnombre =  fncnombeditexs('curso',$iRegcurso,$elementos[0],$elementos[1],'cursocodigo',$iRegcurso[cursocodigo],$nuconn);
					if ($validnombre == 1)
					{
		...
		if($flagerror != 1)
		{
			$result = uprecordcurso($iRegcurso,$nuconn);
			if($result < 0 )
			{
				ob_end_clean();
			...
			if($result > 0)
			{
				...
				echo '<!--//'."\n";
				echo 'location ="maestablcurso.php?codigo='.$codigo.';"';
				echo '//-->'."\n";
				echo '</script>';
			}
			fncclose($nuconn);
...
editacurso($iRegcurso,$flageditarcurso,$campnomb,$codigo);
?> 
