...
Funcion         : grabavaranalisis
Decripcion      : Valida la data a grabar y la lleva al paquete.
Parametros      : Descripicion
$iRegproveedo         Arreglo de datos.
$flagnuevovaranalisis    Bandera de validaci�n
Retorno         :
true	= 1
false	= 0
Autor           : ariascos
...
function grabavaranalisis($iRegvaranalisis,&$flagnuevovaranalisis,&$campnomb,$flagerror)
{
	$nuconn = fncconn();
	...
	do
	{
		$nuresult = loadrecordvaranalisis($nuidtemp,$nuconn);
		if($nuresult == e_empty)
		{
			$iRegvaranalisis[varanacodigo] = $nuidtemp;
		}
		$nuidtemp ++;
	...
	if($iRegvaranalisis)
	{
		...
		if($flagerror != 1)
		{
			...
			$result = insrecordvaranalisis($iRegvaranalisis,$nuconn);
			if($result < 0 )
			{
...
grabavaranalisis($iRegvaranalisis,$flagnuevovaranalisis,$campnomb,$flagerror);
