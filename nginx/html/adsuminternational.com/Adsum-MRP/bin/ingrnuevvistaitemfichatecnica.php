<?php 
	...
	if($accionnuevovistaitemfichatecnica) 
		include ( 'grabavistaitemfichatecnica.php');
	...
	if(!$flagnuevovistaitemfichatecnica)
	{
		...
		include 'cargarcamperfichat1.php';
		//nota hay que asignar la varible producto con el codigo del producto actual. 
		$producto = $produccodigo;
		...
		$respon = $nombre;
		//formula para ancho de impresion
	   	if($tipitecodigo == 6 || $tipitecodigo == 1)
			$ancho_impresion = ($ancho * $nropistas);
		...
		{
			$ancho_impresion = ( ( ($largo + $fuelle) * 2) * $nropistas ) + $refile;
			$largo_impresion = $ancho;
		}
		//evento especial para ancho ideal en productos {capuchon}
		if($tipitecodigo == 5)
			$ancho_impresion = ( ( ($largo + $pestania) * 2) * $nropistas);
		$nombre = cargausuanombre($usuacodi, $idcon);
		$respon = $nombre;
...
<?php endif; ?>
			...
			<input type="hidden" name="accionnuevovistaitemfichatecnica"> 
