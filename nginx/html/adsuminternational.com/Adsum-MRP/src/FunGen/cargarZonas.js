...
function cargarZonas(valor)
{
    ...
    */
    jsrsExecute("procZonas.php", cargarZonasResultado, "mostrarZonas", valor );
}

function cargarZonasResultado(cadena)
{
	if(cadena != "")
	{
	    ...
	    var selected = true;
	    window.document.form1.zona.length = 1;
	    window.document.form1.zona.options[0] = new Option("Seleccione","",defaultSelected, selected);		
	    ...
	    for(i = 0; i < miArray.length -1; i++)
	    {
			...
			valor = miArray[i];
			nombre = miArray[i+1];
			window.document.form1.zona.options[j] = new Option(nombre,valor,defaultSelected, selected);
			j++;
			i += 1;
	    }
	}
	...
	{
		window.document.form1.zona.length = 0;
		var defaultSelected = true;
	    var selected = true;
	    window.document.form1.zona.options[0] = new Option("Seleccione","",defaultSelected, selected);	
	}
