...
function cargarEquipos(valor)
{
    ...
    */
    jsrsExecute("procEquipos.php", cargarEquiposResultado, "mostrarEquipos", valor );
}

function cargarEquiposResultado(cadena)
{
	if(cadena != "")
	{
	    ...
	    var selected = true;
	    window.document.form1.equipocodigo.length = 1;
	    ...
	    for(i = 0; i < miArray.length -1; i++)
	    {
			...
			valor = miArray[i];
			nombre = miArray[i+1];
			window.document.form1.equipocodigo.options[j] = new Option(nombre,valor,defaultSelected, selected);
			j++;
			i += 1;
	    }
	}
