...
function cargarEmpleaselec(valor)
{
    /*
    ...
    */
    jsrsExecute("procEmpleaselec.php", cargarEmpleaselecResultado, "mostrarEmpleaselec", valor );
}

function cargarEmpleaselecResultado(cadena)
{
	if(cadena != "")
	{
	    ...
	    for(i = 0; i < miArray.length -1; i++)
	    {
			...
			valor = miArray[i];
			nombre = miArray[i+1];
			window.document.form1.empleaselec.options[j] = new Option(nombre,valor,defaultSelected, selected);
			j++;
			i += 1;
	    }
	}
