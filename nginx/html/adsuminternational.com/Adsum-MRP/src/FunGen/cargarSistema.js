...
function cargarSistemas(valor)
{
    ...
    */
    ...
    jsrsExecute("procSistemas.php", cargarSistemasResultado, "mostrarSistemas", valor );
}

function cargarSistemasResultado(cadena)
{
	if(cadena != "")
	{
	    ...
	    var selected = true;
	    window.document.form1.sistemcodigo.length = 1;
	    window.document.form1.sistemcodigo.options[0] = new Option(" -- Seleccione --","",defaultSelected, selected);		
	    j = 1;
	    for(i = 0; i < miArray.length -1; i++)
	    {
			...
			valor = miArray[i];
			nombre = miArray[i+1];
			window.document.form1.sistemcodigo.options[j] = new Option(nombre,valor,defaultSelected, selected);
			j++;
			i += 1;
	    }
	}
	...
	{
		window.document.form1.sistemcodigo.length = 0;
		var defaultSelected = true;
	    var selected = true;
	    window.document.form1.sistemcodigo.options[0] = new Option(" -- Seleccione --","",defaultSelected, selected);		
	    
	    window.document.form1.equipocodigo.length = 0;
	    window.document.form1.equipocodigo.options[0] = new Option(" -- Seleccione --","",defaultSelected, selected);		
