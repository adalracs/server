...
function cargarNorSegselec(valor)
{
    ...
    */
    jsrsExecute("procNorSegselec.php", cargarNorSegselecResultado, "mostrarNorSegselec", valor);
}

function cargarNorSegselecResultado(cadena)
{
	if(cadena != "")
	{
	    ...
	    var selected = false;
	    window.document.form1.norsegselec.length = 0;
	    j = 0;
	    for(i = 0; i < miArray.length -1; i++)
	    {
			...
			valor = miArray[i];
			nombre = miArray[i+1];
			window.document.form1.norsegselec.options[j] = new Option(nombre,valor,defaultSelected, selected);
			j++;
			i += 1;
	    }
	}
