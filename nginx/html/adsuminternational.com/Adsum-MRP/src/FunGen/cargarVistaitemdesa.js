...
<!--Fecha           : 25-abr-2005-->
function cargarVistaitemdesa(valor)
{
    /*
    ...
    */
    jsrsExecute("procVistaitemdesa.php", cargarVistaitemdesaResultado, "mostrarVistaitemdesa", valor );
}

function cargarVistaitemdesaResultado(cadena)
{
	if(cadena != "")
	{
	    ...
	    var selected = true;
	    window.document.form1.itedescodigo.length = 1;
	    window.document.form1.itedescodigo.options[0] = new Option(" -- Seleccione --","",defaultSelected, selected);		
	    j = 1;
	    for(i = 0; i < miArray.length -1; i++)
	    {
			...
			valor = miArray[i];
			nombre = miArray[i+1];
			window.document.form1.itedescodigo.options[j] = new Option(nombre,valor,defaultSelected, selected);
			j++;
			i += 1;
	    }
	}
	...
	{
		window.document.form1.itedescodigo.length = 0;
		var defaultSelected = true;
	    var selected = true;
	}
}
