...
function ajaxItems()
{
	$.ajax({	   
		dataType: "html",
		type: "POST",        
		url: "../src/FunjQuery/jquery.phpscripts/jq.ajx.listentrega.php",
		data:	"arroe=" + document.getElementById('arroe').value +
...
function reLoadListItems(){	
	var arrObjItems = document.getElementById('arroe').value.split(','); //el comodin ',' es separador de filas
	var objparams = "arroe=" + document.getElementById('arroe').value +
					"&ordoppcodigo=" + document.getElementById('ordoppcodigo').value +
					"&solprocodigo=" + document.getElementById('solprocodigo').value +
					"&procednombre=" + document.getElementById('procednombre').value +
					"&produccoduno=" + document.getElementById('produccoduno').value +
					"&producnombre=" + document.getElementById('producnombre').value;
	ajaxListaOrdenEntrega(objparams);
}
//evento ajax para recargar items asignados
function ajaxListaOrdenEntrega(objparams)
{
	$.ajax({	   
		dataType: "html",
		type: "POST",        
		url: "../src/FunjQuery/jquery.visors/jq.voe.php",
		data: objparams,
		beforeSend: function(data){
			document.getElementById('listordenentrega').innerHTML = '<img src="../img/loading.gif">&nbsp;Espere mientras carga el formulario.</img>';		
		},         
		success: function(requestData){
			document.getElementById('listordenentrega').innerHTML = requestData;
		},         
		error: function(requestData, strError, strTipoError){ },
		complete: function(requestData, exito){}                                      
	});
