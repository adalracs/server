$(function(){
	//tab's para la bandeja general de valvulado
	...
	//tab's para el programa de valvulado
	$("#programavalvulado").tabs({
		ajaxOptions: {
		error: function(xhr, status, index, anchor) {
		$(anchor.hash).html("No se puede cargar esta pesta&ntilde;a. Vamos a tratar de solucionar este problema lo m&aacute;s pronto posible.");
			}
	...
	/*
	//boton para evento de simular opp de valvulado
	$('#simularopp_vlv').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
	});
	*/
	//boton con evento para geneer opp de valvulado
	$('#generaropp_vlv').button({ icons: { primary: "ui-icon-circle-check" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_vlv" ).button( "option", "disabled", true );
		$( "#backward_vlv" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='ingrnuevopp_vlv.php';
			document.form1.submit();
		}
		else
		{
			$( "#generaropp_vlv" ).button( "option", "disabled", false );
			$( "#backward_vlv" ).button( "option", "disabled", false );
			//mensaje de error
	...
	//boton de atras para retornar a la bandeja de valvulado
	$('#backward_vlv').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_vlv" ).button( "option", "disabled", true );
		$( "#backward_vlv" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablbandejavalvulado.php';
		document.form1.submit();
		return false;
	});
	//boton para ingresar a editar el programa de valvulado		
	$('#editarprograma_vlv').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editarprograma_vlv" ).button( "option", "disabled", true );
		//acccion del boton	
		document.form1.action='_editarprogramavalvulado.php';
		document.form1.submit();
		return false;
	});
	//evento para accion de graba la orden de produccion
	$('#aceptaropp_vlv').button({ icons: { primary: "ui-icon-circle-check" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_vlv" ).button( "option", "disabled", true );
		$( "#cancelaropp_vlv" ).button( "option", "disabled", true );
		//acccion del boton
	...
	$('#cancelaropp_vlv').button({ icons: { primary: "ui-icon-circle-close" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_vlv" ).button( "option", "disabled", true );
		$( "#cancelaropp_vlv" ).button( "option", "disabled", true );
		//acccion del boton
...
//				$(this).dialog("close"); 
	...
	//evento para editar el programa de valvulado
    $('#editaprograma_vlv').button({ icons: { primary: "ui-icon-pencil" } }).click(function() {
        //objetos a utilizar
	...
	$('#cancelarprograma_vlv').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaprograma_vlv" ).button( "option", "disabled", true );
		$( "#cancelarprograma_vlv" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablprogramavalvulado.php';
		document.form1.submit();
		return false;
	});
	
	//boton para evento de simular opp de extrusion
	$('#editaropp_vlv').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaropp_vlv" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='editaropp_vlv.php';
			document.form1.submit();
		}
		else
		{
			$( "#editaropp_vlv" ).button( "option", "disabled", false );
			//mensaje de error
			document.getElementById('msg').innerHTML = err;
			$("#msgwindow").dialog("open");
		}
...
//evento que crear los animatedcollapse en la bandeja de valvulado
...
//evento para actualizar el programa de valvulado
function update_programa(arr_programa)
{
	...
	$.ajax({	   
		dataType: "json",
		type: "POST",        
		url: "../src/FunjQuery/jquery.accionextras/jquery.ajax_editarprogramavlv.php",
		...
		success: function(json){
			...
			$( "#editaprograma_vlv" ).button( "option", "disabled", false );
			$( "#cancelarprograma_vlv" ).button( "option", "disabled", false );
		}
	});
}
