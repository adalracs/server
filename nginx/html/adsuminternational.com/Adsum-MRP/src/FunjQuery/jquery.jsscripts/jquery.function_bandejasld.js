$(function(){
	//tab's para la bandeja general de sellado
	...
	//tab's para el programa de sellado
	$("#programasellado").tabs({
		ajaxOptions: {
		error: function(xhr, status, index, anchor) {
		$(anchor.hash).html("No se puede cargar esta pesta&ntilde;a. Vamos a tratar de solucionar este problema lo m&aacute;s pronto posible.");
			}
	...
	/*
	//boton para evento de simular opp de sellado
	$('#simularopp_sld').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
	});
	*/
	//boton con evento para geneer opp de sellado
	$('#generaropp_sld').button({ icons: { primary: "ui-icon-circle-check" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_sld" ).button( "option", "disabled", true );
		$( "#backward_sld" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='ingrnuevopp_sld.php';
			document.form1.submit();
		}
		else
		{
			$( "#generaropp_sld" ).button( "option", "disabled", false );
			$( "#backward_sld" ).button( "option", "disabled", false );
			//mensaje de error
	...
	//boton de atras para retornar a la bandeja de sellado
	$('#backward_sld').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_sld" ).button( "option", "disabled", true );
		$( "#backward_sld" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablbandejasellado.php';
		document.form1.submit();
		return false;
	});
	//boton para ingresar a editar el programa de sellado		
	$('#editarprograma_sld').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editarprograma_sld" ).button( "option", "disabled", true );
		//acccion del boton	
		document.form1.action='_editarprogramasellado.php';
		document.form1.submit();
		return false;
	});
	//evento para accion de graba la orden de produccion
	$('#aceptaropp_sld').button({ icons: { primary: "ui-icon-circle-check" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_sld" ).button( "option", "disabled", true );
		$( "#cancelaropp_sld" ).button( "option", "disabled", true );
		//acccion del boton
	...
	$('#cancelaropp_sld').button({ icons: { primary: "ui-icon-circle-close" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_sld" ).button( "option", "disabled", true );
		$( "#cancelaropp_sld" ).button( "option", "disabled", true );
		//acccion del boton
...
//				$(this).dialog("close"); 
	...
	//evento para editar el programa de sellado
    $('#editaprograma_sld').button({ icons: { primary: "ui-icon-pencil" } }).click(function() {
        //objetos a utilizar
	...
	$('#cancelarprograma_sld').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaprograma_sld" ).button( "option", "disabled", true );
		$( "#cancelarprograma_sld" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablprogramasellado.php';
		document.form1.submit();
		return false;
	});
	
	//boton para evento de simular opp de extrusion
	$('#editaropp_sld').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaropp_sld" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='editaropp_sld.php';
			document.form1.submit();
		}
		else
		{
			$( "#editaropp_sld" ).button( "option", "disabled", false );
			//mensaje de error
			document.getElementById('msg').innerHTML = err;
			$("#msgwindow").dialog("open");
		}
...
//evento que crear los animatedcollapse en la bandeja de sellado
...
//evento para actualizar el programa de sellado
function update_programa(arr_programa)
{
	...
	$.ajax({	   
		dataType: "json",
		type: "POST",        
		url: "../src/FunjQuery/jquery.accionextras/jquery.ajax_editarprogramasld.php",
		...
		success: function(json){
			...
			$( "#editaprograma_sld" ).button( "option", "disabled", false );
			$( "#cancelarprograma_sld" ).button( "option", "disabled", false );
		}
	});
}
