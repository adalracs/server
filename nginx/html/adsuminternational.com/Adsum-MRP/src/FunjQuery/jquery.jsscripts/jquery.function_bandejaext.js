$(function(){
	...
	//tab's para la bandeja general de extrusion
	...
	//tab's para el programa de extrusion
	$("#programaextrusion").tabs({
		ajaxOptions: {
		error: function(xhr, status, index, anchor) {
		$(anchor.hash).html("No se puede cargar esta pesta&ntilde;a. Vamos a tratar de solucionar este problema lo m&aacute;s pronto posible.");
			}
	...
	$('#simularopp_ext').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#simularopp_ext" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='simularopp_ext.php';
			document.form1.submit();
		}
		else
		{
			$( "#simularopp_ext" ).button( "option", "disabled", false );
			//mensaje de error
	...
	//evento para editar el programa de extrusion
	$('#editarprograma_ext').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editarprograma_ext" ).button( "option", "disabled", true );
		//acccion del boton		
		document.form1.action='_editarprogramaextrusion.php';
		document.form1.submit();
		return false;
	});
	//boton con evento para geneer opp de extrusion
	$('#generaropp_ext').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_ext" ).button( "option", "disabled", true );
		$( "#backward_ext" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='ingrnuevopp_ext.php';
			document.form1.submit();	
		}
		else
		{
			$( "#generaropp_ext" ).button( "option", "disabled", false );
			$( "#backward_ext" ).button( "option", "disabled", false );
			//mensaje de error
	...
	//boton de atras para retornar a la bandeja de extrusion
	$('#backward_ext').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_ext" ).button( "option", "disabled", true );
		$( "#backward_ext" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablbandejaextrusion.php';
		document.form1.submit();
		return false;
	});
	//evento para graba la orden de produccion opp
	$('#aceptaropp_ext').button({ icons: { primary: "ui-icon-circle-check" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_ext" ).button( "option", "disabled", true );
		$( "#cancelaropp_ext" ).button( "option", "disabled", true );
		//acccion del boton
	...
	$('#cancelaropp_ext').button({ icons: { primary: "ui-icon-circle-close" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_ext" ).button( "option", "disabled", true );
		$( "#cancelaropp_ext" ).button( "option", "disabled", true );
		//acccion del boton
...
//				$(this).dialog("close"); 
	...
	//evento para editar el programa de extrusion
	$('#editaprograma_ext').button({ icons: { primary: "ui-icon-pencil" } }).click(function() {
        //objetos a utilizar
	...
	//evento para cancelar la edicion de programa de extrusion
	$('#cancelarprograma_ext').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaprograma_ext" ).button( "option", "disabled", true );
		$( "#cancelarprograma_ext" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablprogramaextrusion.php';
		document.form1.submit();
		return false;
	});
	//boton para evento de simular opp de extrusion
	$('#editaropp_ext').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaropp_ext" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='editaropp_ext.php';
			document.form1.submit();
		}
		else
		{
			$( "#editaropp_ext" ).button( "option", "disabled", false );
			//mensaje de error
			document.getElementById('msg').innerHTML = err;
			$("#msgwindow").dialog("open");
		}
...
function ajax_filtro()
{
	...
	var obj_ordproancext = document.getElementById('ordproancext');
	...
	var ordproancext = (obj_ordproancext)? obj_ordproancext.value : '' ; 
	...
	parameters = (ordproancext != '')? parameters + '&ordproancext=' + ordproancext : parameters + '&ordproancext=' ;
	...
	$.ajax({	   
		...
		beforeSend: function(data){ 
			//document.getElementById('detallebandejaextrusion').innerHTML = '<img src="../img/loading.gif">&nbsp;Espere mientras carga la bandeja.</img>';
			$('#detallebandejaextrusion').block({ theme : true , message : '<img src="../img/loading.gif">&nbsp;Espere mientras carga la bandeja.</img>'});
		...
		{	
			$('#detallebandejaextrusion').unblock();
			//objetos a usar
...
function ajax_filtroopp()
{
	...
	var obj_ordproancext = document.getElementById('ordproancext_opp');
	...
	$.ajax({	   
		dataType: "html",
		type: "POST",        
		url: "detallaordenextrusion.php",
		data: parameters,
		beforeSend: function(data){ 
			$('#detalleordenextrusion').block({ theme : true , message : '<img src="../img/loading.gif">&nbsp;Espere mientras carga las ordenes de produccion.</img>'});
			//document.getElementById('detalleordenextrusion').innerHTML = '<img src="../img/loading.gif">&nbsp;Espere mientras carga las ordenes de produccion.</img>';
		...
		success: function(requestData){
			document.getElementById('detalleordenextrusion').innerHTML = requestData;
		},
		complete : function(requestData)
		{		
			$('#detalleordenextrusion').unblock();
			//objetos a usar
...
//evento para actualizar el programa de extrusion
function update_programa(arr_programa)
{
	...
	$.ajax({	   
		dataType: "json",
		type: "POST",        
		url: "../src/FunjQuery/jquery.accionextras/jquery.ajax_editarprogramaext.php",
		...
		success: function(json){
			...
			$( "#editaprograma_ext" ).button( "option", "disabled", false );
			$( "#cancelarprograma_ext" ).button( "option", "disabled", false );
		}
	});
}
...
function event_producatc(indice)
{
	...
	//autocomplete material para item de produccion en extrusion
...
function reladlistVelocidadpn()
{
	$.ajax({	   
		...
		beforeSend: function(data){ 
			//document.getElementById('listavelocidadpn').innerHTML = '<img src="../img/loading.gif">&nbsp;Espere mientras cargan las velocidades de produccion.</img>';
