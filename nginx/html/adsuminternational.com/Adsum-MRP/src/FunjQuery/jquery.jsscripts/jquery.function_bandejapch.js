$(function(){
	//tab's para la bandeja general de pauchado
	...
	//tab's para el programa de pauchado
	$("#programapauchado").tabs({
		ajaxOptions: {
		error: function(xhr, status, index, anchor) {
		$(anchor.hash).html("No se puede cargar esta pesta&ntilde;a. Vamos a tratar de solucionar este problema lo m&aacute;s pronto posible.");
			}
	...
	/*
	//boton para evento de simular opp de pauchado
	$('#simularopp_pch').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
	});
	*/
	//boton con evento para geneer opp de pauchado
	$('#generaropp_pch').button({ icons: { primary: "ui-icon-circle-check" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_pch" ).button( "option", "disabled", true );
		$( "#backward_pch" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='ingrnuevopp_pch.php';
			document.form1.submit();
		}
		else
		{
			$( "#generaropp_pch" ).button( "option", "disabled", false );
			$( "#backward_pch" ).button( "option", "disabled", false );
			//mensaje de error
	...
	//boton de atras para retornar a la bandeja de pauchado
	$('#backward_pch').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_pch" ).button( "option", "disabled", true );
		$( "#backward_pch" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablbandejapauchado.php';
		document.form1.submit();
		return false;
	});
	//boton para ingresar a editar el programa de pauchado		
	$('#editarprograma_pch').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editarprograma_pch" ).button( "option", "disabled", true );
		//acccion del boton	
		document.form1.action='_editarprogramapauchado.php';
		document.form1.submit();
		return false;
	});
	//evento para accion de graba la orden de produccion
	$('#aceptaropp_pch').button({ icons: { primary: "ui-icon-circle-check" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_pch" ).button( "option", "disabled", true );
		$( "#cancelaropp_pch" ).button( "option", "disabled", true );
		//acccion del boton
	...
	$('#cancelaropp_pch').button({ icons: { primary: "ui-icon-circle-close" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_pch" ).button( "option", "disabled", true );
		$( "#cancelaropp_pch" ).button( "option", "disabled", true );
		//acccion del boton
...
//				$(this).dialog("close"); 
	...
	//evento para editar el programa de pauchado
    $('#editaprograma_pch').button({ icons: { primary: "ui-icon-pencil" } }).click(function() {
        //objetos a utilizar
	...
	$('#cancelarprograma_pch').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaprograma_pch" ).button( "option", "disabled", true );
		$( "#cancelarprograma_pch" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablprogramapauchado.php';
		document.form1.submit();
		return false;
	});
	
	//boton para evento de simular opp de extrusion
	$('#editaropp_pch').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaropp_pch" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='editaropp_pch.php';
			document.form1.submit();
		}
		else
		{
			$( "#editaropp_pch" ).button( "option", "disabled", false );
			//mensaje de error
			document.getElementById('msg').innerHTML = err;
			$("#msgwindow").dialog("open");
		}
...
//evento que crear los animatedcollapse en la bandeja de pauchado
...
//evento para actualizar el programa de pauchado
function update_programa(arr_programa)
{
	...
	$.ajax({	   
		dataType: "json",
		type: "POST",        
		url: "../src/FunjQuery/jquery.accionextras/jquery.ajax_editarprogramapch.php",
		...
		success: function(json){
			...
			$( "#editaprograma_pch" ).button( "option", "disabled", false );
			$( "#cancelarprograma_pch" ).button( "option", "disabled", false );
		}
	});
}
