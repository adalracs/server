$(function(){
	//tab's para la bandeja general de doblado
	...
	//tab's para el programa de doblado
	$("#programadoblado").tabs({
		ajaxOptions: {
		error: function(xhr, status, index, anchor) {
		$(anchor.hash).html("No se puede cargar esta pesta&ntilde;a. Vamos a tratar de solucionar este problema lo m&aacute;s pronto posible.");
			}
	...
	/*
	//boton para evento de simular opp de doblado
	$('#simularopp_dbl').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
	});
	*/
	//boton con evento para geneer opp de doblado
	$('#generaropp_dbl').button({ icons: { primary: "ui-icon-circle-check" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_dbl" ).button( "option", "disabled", true );
		$( "#backward_dbl" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='ingrnuevopp_dbl.php';
			document.form1.submit();
		}
		else
		{
			$( "#generaropp_dbl" ).button( "option", "disabled", false );
			$( "#backward_dbl" ).button( "option", "disabled", false );
			//mensaje de error
	...
	//boton de atras para retornar a la bandeja de doblado
	$('#backward_dbl').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_dbl" ).button( "option", "disabled", true );
		$( "#backward_dbl" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablbandejadoblado.php';
		document.form1.submit();
		return false;
	});
	//boton para ingresar a editar el programa de doblado		
	$('#editarprograma_dbl').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editarprograma_dbl" ).button( "option", "disabled", true );
		//acccion del boton	
		document.form1.action='_editarprogramadoblado.php';
		document.form1.submit();
		return false;
	});
	//evento para accion de graba la orden de produccion
	$('#aceptaropp_dbl').button({ icons: { primary: "ui-icon-circle-check" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_dbl" ).button( "option", "disabled", true );
		$( "#cancelaropp_dbl" ).button( "option", "disabled", true );
		//acccion del boton
	...
	$('#cancelaropp_dbl').button({ icons: { primary: "ui-icon-circle-close" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_dbl" ).button( "option", "disabled", true );
		$( "#cancelaropp_dbl" ).button( "option", "disabled", true );
		//acccion del boton
...
//				$(this).dialog("close"); 
	...
	//evento para editar el programa de doblado
    $('#editaprograma_dbl').button({ icons: { primary: "ui-icon-pencil" } }).click(function() {
        //objetos a utilizar
	...
	$('#cancelarprograma_dbl').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaprograma_dbl" ).button( "option", "disabled", true );
		$( "#cancelarprograma_dbl" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablprogramadoblado.php';
		document.form1.submit();
		return false;
	});
	
	//boton para evento de simular opp de extrusion
	$('#editaropp_dbl').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaropp_dbl" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='editaropp_dbl.php';
			document.form1.submit();
		}
		else
		{
			$( "#editaropp_dbl" ).button( "option", "disabled", false );
			//mensaje de error
			document.getElementById('msg').innerHTML = err;
			$("#msgwindow").dialog("open");
		}
...
//evento que crear los animatedcollapse en la bandeja de doblado
...
//evento para actualizar el programa de doblado
function update_programa(arr_programa)
{
	...
	$.ajax({	   
		dataType: "json",
		type: "POST",        
		url: "../src/FunjQuery/jquery.accionextras/jquery.ajax_editarprogramadbl.php",
		...
		success: function(json){
			...
			$( "#editaprograma_dbl" ).button( "option", "disabled", false );
			$( "#cancelarprograma_dbl" ).button( "option", "disabled", false );
		}
	});
}
