$(function(){
	//tab's para la bandeja general de troquelado
	...
	//tab's para el programa de troquelado
	$("#programatroquelado").tabs({
		ajaxOptions: {
		error: function(xhr, status, index, anchor) {
		$(anchor.hash).html("No se puede cargar esta pesta&ntilde;a. Vamos a tratar de solucionar este problema lo m&aacute;s pronto posible.");
			}
	...
	/*
	//boton para evento de simular opp de troquelado
	$('#simularopp_tql').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
	});
	*/
	//boton con evento para geneer opp de troquelado
	$('#generaropp_tql').button({ icons: { primary: "ui-icon-circle-check" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_tql" ).button( "option", "disabled", true );
		$( "#backward_tql" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='ingrnuevopp_tql.php';
			document.form1.submit();
		}
		else
		{
			$( "#generaropp_tql" ).button( "option", "disabled", false );
			$( "#backward_tql" ).button( "option", "disabled", false );
			//mensaje de error
	...
	//boton de atras para retornar a la bandeja de troquelado
	$('#backward_tql').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#generaropp_tql" ).button( "option", "disabled", true );
		$( "#backward_tql" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablbandejatroquelado.php';
		document.form1.submit();
		return false;
	});
	//boton para ingresar a editar el programa de troquelado		
	$('#editarprograma_tql').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editarprograma_tql" ).button( "option", "disabled", true );
		//acccion del boton	
		document.form1.action='_editarprogramatroquelado.php';
		document.form1.submit();
		return false;
	});
	//evento para accion de graba la orden de produccion
	$('#aceptaropp_tql').button({ icons: { primary: "ui-icon-circle-check" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_tql" ).button( "option", "disabled", true );
		$( "#cancelaropp_tql" ).button( "option", "disabled", true );
		//acccion del boton
	...
	$('#cancelaropp_tql').button({ icons: { primary: "ui-icon-circle-close" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#aceptaropp_tql" ).button( "option", "disabled", true );
		$( "#cancelaropp_tql" ).button( "option", "disabled", true );
		//acccion del boton
...
//				$(this).dialog("close"); 
	...
	//evento para editar el programa de troquelado
    $('#editaprograma_tql').button({ icons: { primary: "ui-icon-pencil" } }).click(function() {
        //objetos a utilizar
	...
	$('#cancelarprograma_tql').button({ icons: { primary: "ui-icon-seek-prev" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaprograma_tql" ).button( "option", "disabled", true );
		$( "#cancelarprograma_tql" ).button( "option", "disabled", true );
		//acccion del boton
		document.form1.action='maestablprogramatroquelado.php';
		document.form1.submit();
		return false;
	});
	
	//boton para evento de simular opp de extrusion
	$('#editaropp_tql').button({ icons: { primary: "ui-icon-gear" } }).click(function() {
		//evento de desactivar botones mientras realiza el submit
		$( "#editaropp_tql" ).button( "option", "disabled", true );
		//objetos a utilizar
		...
		{
			//acccion del boton
			document.form1.action='editaropp_tql.php';
			document.form1.submit();
		}
		else
		{
			$( "#editaropp_tql" ).button( "option", "disabled", false );
			//mensaje de error
			document.getElementById('msg').innerHTML = err;
			$("#msgwindow").dialog("open");
		}
...
//evento que crear los animatedcollapse en la bandeja de troquelado
...
//evento para actualizar el programa de troquelado
function update_programa(arr_programa)
{
	...
	$.ajax({	   
		dataType: "json",
		type: "POST",        
		url: "../src/FunjQuery/jquery.accionextras/jquery.ajax_editarprogramatql.php",
		...
		success: function(json){
			...
			$( "#editaprograma_tql" ).button( "option", "disabled", false );
			$( "#cancelarprograma_tql" ).button( "option", "disabled", false );
		}
	});
}
