<?php
	...
	$sql = "select distinct(ordcomcodcli), ordcomrazsoc from ordencompra WHERE upper(ordcomrazsoc) LIKE upper('%".$termino."%')";
//	$rs_item = dinamicscanopitem($record, $recordop, $idcon);
	...
	if($numReg)
	{
		for ($i = 0; $i < $numReg; $i++)
		{
			...
			array_push($result, array("id" => $arr['ordcomcodcli'], "label" => rtrim($arr['ordcomrazsoc']), "value" => rtrim($arr['ordcomrazsoc'])));
