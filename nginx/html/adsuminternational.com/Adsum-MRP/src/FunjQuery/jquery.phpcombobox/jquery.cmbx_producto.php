<?php
	...
	$sql = "select distinct(produccoduno),producnombre from producto WHERE upper(producnombre) LIKE upper('%".$termino."%') OR upper(produccoduno) LIKE upper('%".$termino."%')";
//	$rs_item = dinamicscanopitem($record, $recordop, $idcon);
	...
	if($numReg)
	{
		for ($i = 0; $i < $numReg; $i++)
		{
			...
			array_push($result, array("id" => $arr['produccoduno'], "label" => rtrim($arr['producnombre']), "value" => rtrim($arr['producnombre'])));
