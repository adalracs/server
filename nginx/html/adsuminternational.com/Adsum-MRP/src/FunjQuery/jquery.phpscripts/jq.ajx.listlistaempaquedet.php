...
ini_set('display_errors',1);
	...
	function innerHTML (&$html, $sbRow, $tipmat, $swth_sel,$sbarray)
	{
		...
		$html .= '<tr><td colspan="2" class="ui-widget-header estilo2"><input type="checkbox" name="chkselstar" value="'.$sbarray.'" '.$swth_sel.' onclick="'."setSelectionRow(this.value, document.getElementById('arrlistaempaquedet').value, ',', 'arrlistaempaquedet');".'">&nbsp;'.$sbRow['idmat'].'-'.$sbRow['nmat'].'</input></td></tr>'."\n";
	...
	if($arrlistaempaquedet)
	{
		$array_tmp = explode(',',$arrlistaempaquedet);
	...
	for($a = 0; $a < $nrReporteopp; $a++)
	{
		...
		for($b = 0; $b < $nrReporteoppreportepn; $b++)
		{
			...
			innerHTML($reporteLISTAEMPAQUEDET, $rwReport, 0, $swth_sel, $rwReporteoppreportepn['reoppncodigo']);
		}
	}	
?>
