...
ini_set('display_errors',1);
	...
	function innerHTML (&$html, $sbRow, $tipmat, $swth_sel,$sbarray)
	{
		...
		$html .= '<tr><td colspan="2" class="ui-widget-header estilo2"><input type="checkbox" name="chkselstar" value="'.$sbarray.'" '.$swth_sel.' onclick="'."setSelectionRow(this.value, document.getElementById('arrlistaempaque').value, ',', 'arrlistaempaque');".'">&nbsp;'.$sbRow['idmat'].'-'.$sbRow['nmat'].'</input></td></tr>'."\n";
	...
	if($arrlistaempaque)
	{
		$array_tmp = explode(',',$arrlistaempaque);
	...
	for($a = 0; $a < $nrReporteopp; $a++)
	{
		...
		for($b = 0; $b < $nrReporteoppreportepn; $b++)
		{
			...
			innerHTML($reporteLISTAEMPAQUE, $rwReport, 0, $swth_sel, $rwReporteoppreportepn['reoppncodigo']);
		}
	}	
?>
