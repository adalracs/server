...
function Grid(element, options, methods, viewName) {
	...
	function renderGrid(r, c, colFormat, showNumbers) {
		...
		if (!tbody) { // first time, build all cells from scratch
			...
			s = "<thead><tr>";
			for (i=0; i<colCnt; i++) {
				s += "<th class='fc-" + dayIDs[d.getDay()] + ' ' + // needs to be first
					tm + '-state-default' +
					(i==dit ? ' fc-leftmost' : '') +
			...
			for (i=0; i<rowCnt; i++) {
				...
				for (j=0; j<colCnt; j++) {
					...
					for (k=0; k<festiv.length; k++){
						var df = transStringtoDate(festiv[k], d.getFullYear());
						
						if(d.getDate() == df.getDate() && d.getMonth() == df.getMonth() && d.getFullYear() == df.getFullYear())
						{
							rlfestiv = true;
							break;
						}
					...
					s += "<td class='fc-" +
						...
						(j==dit ? ' fc-leftmost' : '') +
						(rowCnt > 1 && d.getMonth() != month ? ' fc-other-month' : '') + (dayIDs[d.getDay()] == 'dom' || rlfestiv == true ?  ' fc-festiv' : '' ) +
				...
				for (i=prevRowCnt; i<rowCnt; i++) {
					...
					for (j=0; j<colCnt; j++) {
						...
						s += "<td class='fc-" +
							...
							(j==dit ? ' fc-leftmost' : '') + + (dayIDs[d.getDay()] == 'dom' || rlfestiv == true ?  ' fc-festiv' : '' ) + "'>" +
			...
			tbody.find('td').each(function() {
				...
				for (k=0; k<festiv.length; k++){
					...
					if(d.getDate() == df.getDate() && d.getMonth() == df.getMonth() && d.getFullYear() == df.getFullYear())
					{
						td.addClass('fc-festiv');
						break;
					}
					else
					{
						if (dayIDs[d.getDay()] != 'dom')
							td.removeClass('fc-festiv');
					}
				}
...
function _renderDaySegs(segs, rowCnt, view, minLeft, maxLeft, getRow, dayContentLeft, dayContentRight, segmentContainer, bindSegHandlers, modifiedEventId) {
	...
	for (i=0; i<segCnt; i++) {
		...
		className = 'fc-event' + old + ' fc-event-hori ';
		if (rtl) {
			if (seg.isStart) {
...
function Agenda(element, options, methods, viewName) {
	...
	function renderSlotSegs(segs, modifiedEventId) {
		...
		for (i=0; i<segCnt; i++) {
			...
			className = 'fc-event' + old + ' fc-event-vert ';
			if (seg.isStart) {
	...
	function slotSegHtml(event, seg, className) {
		...
		return "<div class='" + className + event.className.join(' ') + "' style='position:absolute;z-index:8;top:" + seg.top + "px;left:" + seg.left + "px'>" +
			"<a" + (event.url ? " href='" + htmlEscape(event.url) + "'" : '') + ">" +
				...
				"<span class='fc-event-time" + old + "'>" + htmlEscape(formatDates(event.start, event.end, view.option('timeFormat'))) + "</span>" +
				"<span class='fc-event-title" + old + "'>" + htmlEscape(event.title) + "</span>" +
			"</a>" +
			((event.editable || event.editable === undefined && options.editable) && !options.disableResizing && $.fn.resizable ?
...
/**
 * Funcion transStringtoDate 
 * convierte una fecha en formato ('dd/mm/yyyy', 'dd-mm-yyyy', 'dd.mm.yyyy') a un objeto Date() para asi
 * poder trabajar con las propiedades de este objeto
 */
function transStringtoDate(string, year) 
{
	var arrStr_dott = string.split("||");
	var strdate = arrStr_dott[0];

	var arrStr_date = strdate.split(strdate.substring(2,3));
	var milsegen = parseInt(24 * 60 * 60 * 1000);
	var numDays = 0;
	var off = false;
	var date = new Date();
	...
	date.setDate(arrStr_date[0]);
	
	if(arrStr_date[2] == '')
		date.setYear(year);
	else
		date.setYear(arrStr_date[2]);
	...
	if(arrStr_dott[1] == '1')
	{
		...
		tiempo = date.getTime();
		total = date.setTime(parseInt(tiempo + (milsegen * numDays)));
	}
