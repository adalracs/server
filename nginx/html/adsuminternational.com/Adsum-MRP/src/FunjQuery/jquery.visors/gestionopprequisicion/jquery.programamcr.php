<?php
	//conexion
	$idcon = fncconn();
	$rsMicroperforado = dinamicscanopprogramamicroperforado1(array("equipocodigo" => $equipo, "ordoppcomfir" => 1),array("equipocodigo" => "=", "ordoppcomfir" => "="),$idcon, $rtr = 1);
...
<?php 
		for($b = 0; $b < $nrMicroperforado; $b++):
			...
			$rsOppVelocidadpn = dinamicscanopoppvelocidadpn(array('ordoppcodigo' => $rwMicroperforado['ordoppcodigo']),array('ordoppcodigo' => '='),$idcon);
			...
			$rsOppAjustepn = dinamicscanopoppajustepn(array('ordoppcodigo' => $rwMicroperforado['ordoppcodigo']),array('ordoppcodigo' => '='),$idcon);
			...
			$rsOppItemDesa = dinamicscanopoppitemdesa(array('ordoppcodigo' => $rwMicroperforado['ordoppcodigo']),array('ordoppcodigo' => '='),$idcon);
			...
			for($c = 0; $c < $nrOppItemDesa; $c++)
			{
				...
				if($rWOppItemDesa['itedescodigo']){
					...
					$MATMICROPERFORADO = ($MATMICROPERFORADO)? $MATMICROPERFORADO.'-&nbsp;'.strtoupper($rWOppItemDesa['itedescodigo']).' - '.carganombitemdesa($rWOppItemDesa['itedescodigo'],$idcon) : strtoupper($rWOppItemDesa['itedescodigo']).' - '.carganombitemdesa($rWOppItemDesa['itedescodigo'],$idcon) ;
				}
			}
			...
			$rsRequisicion = fullscanrequisicionoppxopp( $rwMicroperforado["ordoppcodigo"],$idcon);
			...
			$rwGestionopp = loadrecordultimagestionopp($rwMicroperforado["ordoppcodigo"],$idcon);
...
?>			
	<tr <?php echo $complement; ?> >
		...
		<td width="5%" class="cont-line">&nbsp;<input type="checkbox" name="checkbox1" id="checkbox1" value="<?php echo $rwMicroperforado['ordoppcodigo']; ?>" onclick="setArrrequisicionopp(this.value);" /></td>
		...
		<td width="5%" class="cont-line">&nbsp;<font color="blue"><b><?php echo str_pad($rwOpmicroperforado['solprocodigo'], 4, "0", STR_PAD_LEFT) ?></b></font></td>
		<td width="15%" class="cont-line">&nbsp;<?php echo ($MATMICROPERFORADO)? $MATMICROPERFORADO : "---"; ?></td>
		<td width="10%" class="cont-line">&nbsp;<font color="green"><b><?php echo number_format($KILOSMICROPERFORADO, 2, ',', '.'); ?></b></font></td>
		<td width="10%" class="cont-line">&nbsp;<font color="green"><b><?php echo number_format($METROMICROPERFORADO, 2, ',', '.'); ?></b></font></td>
		<td width="10%" class="cont-line">&nbsp;<?php echo ($rwOpmicroperforado['opestacodigo'])? cargaopestanombre($rwOpmicroperforado['opestacodigo'], $idcon) : "---"; ?></td>
