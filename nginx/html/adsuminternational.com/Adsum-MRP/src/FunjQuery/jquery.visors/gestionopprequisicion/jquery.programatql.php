...
<?php 
		for($b = 0; $b < $nrTroquelado; $b++):
			...
			$rsOppVelocidadpn = dinamicscanopoppvelocidadpn(array('ordoppcodigo' => $rwTroquelado['ordoppcodigo']),array('ordoppcodigo' => '='),$idcon);
			...
			$rsOppAjustepn = dinamicscanopoppajustepn(array('ordoppcodigo' => $rwTroquelado['ordoppcodigo']),array('ordoppcodigo' => '='),$idcon);
			...
			$rsOppItemDesa = dinamicscanopoppitemdesa(array('ordoppcodigo' => $rwTroquelado['ordoppcodigo']),array('ordoppcodigo' => '='),$idcon);
			...
			for($c = 0; $c < $nrOppItemDesa; $c++)
			{
				...
				if($rWOppItemDesa['itedescodigo']){
					...
					$MATTROQUELADO = ($MATTROQUELADO)? $MATTROQUELADO.'-&nbsp;'.strtoupper($rWOppItemDesa['itedescodigo']).' - '.carganombitemdesa($rWOppItemDesa['itedescodigo'],$idcon) : strtoupper($rWOppItemDesa['itedescodigo']).' - '.carganombitemdesa($rWOppItemDesa['itedescodigo'],$idcon) ;
				}
			}
			...
			$rsRequisicion = fullscanrequisicionoppxopp( $rwTroquelado["ordoppcodigo"],$idcon);
			...
			$rwGestionopp = loadrecordultimagestionopp($rwTroquelado["ordoppcodigo"],$idcon);
...
?>			
	<tr <?php echo $complement; ?> >
		...
		<td width="5%" class="cont-line">&nbsp;<input type="checkbox" name="checkbox1" id="checkbox1" value="<?php echo $rwTroquelado['ordoppcodigo']; ?>" onclick="setArrrequisicionopp(this.value);" /></td>
		...
		<td width="5%" class="cont-line">&nbsp;<font color="blue"><b><?php echo str_pad($rwOptroquelado['solprocodigo'], 4, "0", STR_PAD_LEFT) ?></b></font></td>
		<td width="15%" class="cont-line">&nbsp;<?php echo ($MATTROQUELADO)? $MATTROQUELADO : "---"; ?></td>
		<td width="10%" class="cont-line">&nbsp;<font color="green"><b><?php echo number_format($KILOSTROQUELADO, 2, ',', '.'); ?></b></font></td>
		<td width="10%" class="cont-line">&nbsp;<font color="green"><b><?php echo number_format($METROSTROQUELADO, 2, ',', '.'); ?></b></font></td>
		<td width="10%" class="cont-line">&nbsp;<?php echo ($rwOptroquelado['opestacodigo'])? cargaopestanombre($rwOptroquelado['opestacodigo'], $idcon) : "---"; ?></td>
