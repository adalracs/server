<?php
	//conexion
	$idcon = fncconn();
	$rsValvulado = dinamicscanopprogramavalvulado1(array("equipocodigo" => $equipo, "ordoppcomfir" => 1),array("equipocodigo" => "=", "ordoppcomfir" => "="),$idcon, $rtr = 1);
...
<script type="text/javascript">
	Event_animatedcollapse('<?php echo $nrValvulado ?>', 'filtrOpp_<?php echo $equipo ?>');
</script>
...
<?php 
		for($b = 0; $b < $nrValvulado; $b++):
			...
			$$total_equipokgs = $$total_equipokgs + $rwValvulado['ordoppcantkg'];
			$$total_equipomts = $$total_equipomts + $rwValvulado['ordoppcantmt'];

			//escaneo de velocidades para sumatoria de minutos de duracion de la orden
			$rsOppVelocidadpn = dinamicscanopoppvelocidadpn(array('ordoppcodigo' => $rwValvulado['ordoppcodigo']),array('ordoppcodigo' => '='),$idcon);
			...
			$rsOppAjustepn = dinamicscanopoppajustepn(array('ordoppcodigo' => $rwValvulado['ordoppcodigo']),array('ordoppcodigo' => '='),$idcon);
			...
			$rsOppItemDesa = dinamicscanopoppitemdesa(array('ordoppcodigo' => $rwValvulado['ordoppcodigo']),array('ordoppcodigo' => '='),$idcon);
			...
			for($c = 0; $c < $nrOppItemDesa; $c++)
			{
				...
				if($rWOppItemDesa['itedescodigo']){
					...
					$MATVALVULADO = ($MATVALVULADO)? $MATVALVULADO.'-&nbsp;'.strtoupper($rWOppItemDesa['itedescodigo']).' - '.carganombitemdesa($rWOppItemDesa['itedescodigo'],$idcon) : strtoupper($rWOppItemDesa['itedescodigo']).' - '.carganombitemdesa($rWOppItemDesa['itedescodigo'],$idcon) ;
				}
			}
			...
			$APROBADOVALVULADO = ($rwValvulado['ordoppcomfir'] > 0)? '<font color="#000080"><b>Si</b></font>' : '<font color="#FF0000"><b>No</b></font>' ;
			...
			$rsRequisicion = fullscanrequisicionoppxopp( $rwValvulado["ordoppcodigo"],$idcon);
			...
			$rwGestionopp = loadrecordultimagestionopp($rwValvulado["ordoppcodigo"],$idcon);
...
?>			
	<tr <?php echo $complement; ?> >
		...
		<td width="5%" class="cont-line">&nbsp;<input type="checkbox" name="checkbox1" id="checkbox1" value="<?php echo $rwValvulado['ordoppcodigo']; ?>" onclick="setArrrequisicionopp(this.value);" /></td>
		...
		<td width="5%" class="cont-line">&nbsp;<font color="blue"><b><?php echo str_pad($rwOpvalvulado['solprocodigo'], 4, "0", STR_PAD_LEFT) ?></b></font></td>
		<td width="15%" class="cont-line">&nbsp;<?php echo ($MATVALVULADO)? $MATVALVULADO : "---"; ?></td>
		<td width="10%" class="cont-line">&nbsp;<font color="green"><b><?php echo number_format($KILOSVALVULADO, 2, ',', '.'); ?></b></font></td>
		<td width="10%" class="cont-line">&nbsp;<font color="green"><b><?php echo number_format($METROSVALVULADO, 2, ',', '.'); ?></b></font></td>
		<td width="10%" class="cont-line">&nbsp;<?php echo ($rwOpvalvulado['opestacodigo'])? cargaopestanombre($rwOpvalvulado['opestacodigo'], $idcon) : "---"; ?></td>
