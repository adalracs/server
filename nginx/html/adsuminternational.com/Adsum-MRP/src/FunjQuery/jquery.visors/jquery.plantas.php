<?php 
	...
	if($usuaplantareport):
		$sbSql = "	SELECT * FROM planta WHERE plantacodigo IN ({$usuaplanta})";
		$rsPlanta = fncsqlrun($sbSql, $idcon);
		$nrPlanta = fncnumreg($rsPlanta);
	else:
		$rsPlanta = fullscanplanta($idcon);
		$nrPlanta = fncnumreg($rsPlanta);
	endif;
?>	
...
<div style="width:648px; height: 20px;" class="ui-state-default">
	<div style="width:100%; height: auto;">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">	
			<tr>
				<td width="50" class="ui-state-default" style="border-top:0; border-bottom:0; border-left:0;">&nbsp;Sel<input type="checkbox" onclick="setSelectionAll('usuaplanta',','); <?php if($usuaplantareport): ?>rldSubfunction();<?php endif ?> <?php if($usuaplantareportop): ?>reloadSistema();<?php endif ?> " value="1" name="allusuaplanta" id="allusuaplanta"  <?php if($usuaplantareportop): ?>reloadSistema();<?php endif ?> <?php if($allusuaplanta) echo 'checked'; ?> ></td>
...
?>			
			<tr <?php echo $complement ?>">
				<td width="50" style=" border-bottom: 1px solid #fff;"><input type="checkbox" id="chkusuaplanta" name="chkusuaplanta" <?php echo $checked ?> onclick="setSelectionRow(this.value, document.getElementById('arrusuaplanta').value, ',', 'usuaplanta'); <?php if($usuaplantareport): ?>rldSubfunction();<?php endif ?> <?php if($usuaplantareportop): ?>reloadSistema();<?php endif ?>" value="<?php echo $rwPlanta['plantacodigo'] ?>"></td>
