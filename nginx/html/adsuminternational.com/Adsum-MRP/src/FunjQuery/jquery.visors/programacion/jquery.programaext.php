<?php
	$idcon = fncconn();
	$rsExtrusion = dinamicscanopprogramaextrusion1(array('equipocodigo' => $equipo),array('equipocodigo' => '='),$idcon, $rtr = 1);
...
<?php 
		for($b = 0; $b < $nrExtrusion; $b++)
		{
			...
			//echo "<script type='text/javascript'>$('a[title]').qtip({show: 'mouseover',hide: 'mouseout',style: {  border: { width: 5, radius: 5 }, padding: 2, textAlign: 'left', tip: true, name: 'cream' },position: { corner: { target: 'leftMiddle', tooltip: 'rightMiddle' } }});</script>";		
...
?>		
	<tr <?php echo $complement; ?> >
		...
		<td width="5%" class="cont-line">&nbsp;<?php echo ($b + 1); ?>&nbsp;<a onclick="openUpdateOpp('<?php echo $rwExtrusion['ordoppcodigo']; ?>', '<?php echo $equipo ?>');"><span class="ui-icon ui-icon-document" style="float: left; margin-right: .3em;"></span></a></td>
		<td width="5%" class="cont-line">&nbsp;<font color="blue">
			<a href="#" title="Kgs Pn [<?php echo number_format($SZreoppncantkg, 2, ",", "."); ?>] - Kgs Pdt [<?php echo number_format($KILOSEXTRUSION - $SZreoppncantkg, 2, ",", "."); ?>]" style="text-decoration:none;">
				<b><?php echo str_pad($rwExtrusion['ordoppcodigo'], 4, "0", STR_PAD_LEFT); ?></b></font>
			</a>
		</td>
