<?php 
	...
	if($itedescodigo > 0){
		...
		$itedesespeci = $rwItemDesa["itedesespeci"];
...
<?php 
		...
		for($a = 0; $a < $nrVarAnalisis; $a++){
			...
			else if($rwVarAnalisis["varanatipespe"] == 5){

				$vartipoespecificacion = "Tol. + ".$rwVarAnalisis["varanatolems"]." - ".$rwVarAnalisis["varanatolemn"]." (%)";

				if($itedesespeci){
					$vartipoespecificacion = $vartipoespecificacion." - ".$itedesespeci;
					$varanaespecificacion = "{+".($itedesespeci + ($itedesespeci * ($rwVarAnalisis["varanatolems"]) / 100 ))." -".($itedesespeci - ($itedesespeci * ($rwVarAnalisis["varanatolems"]) / 100))."}";
				}
...
?>
			<tr <?php echo $complement ?> >
				...
				<td width="10%" style="border-left: 1px solid #fff; border-bottom: 1px solid #fff;">&nbsp;
					<?php if(!$flagDetallar){ ?>
						<input type="text" name="<?php echo $varValor; ?>" id="<?php echo $varValor; ?>" value="<?php echo $$varValor; ?>" size="5" class="<?php if($campnomb[$varValor] == 1){ echo 'ui-state-error ui-corner-all'; }else if($campnombre[$varValor] == 1){ echo "ui-state-highlight ui-corner-all"; } ?>" onkeyup="valResultado(this.value, '<?php echo $rwVarAnalisis['varanacodigo']; ?>', '<?php echo $itedesespeci; ?>');" /></td>
					<?php }else{ 
