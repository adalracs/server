...
<?php 
	...
	if($arrlistaempaquedet) $arrObjslistaempaquedet = explode(',',$arrlistaempaquedet);
	for($a = 0;$a< count($arrObjslistaempaquedet);$a++){
		$rwReporteoporeportepn = loadrecordreporteoppreportepn($arrObjslistaempaquedet[$a],$idcon);
...
?>
			<tr <?php echo $complement ?>">
				<td width="30" style=" border-bottom: 1px solid #fff;"><?php if(!$flagdetallar):?><input type="checkbox" name="chkarrlistaempaquedet" id="chkarrlistaempaquedet" value="<?php echo $arrObjslistaempaquedet[$a] ?>" onclick="setSelectionRow(this.value, document.getElementById('arrlistaempaquedettmp').value, ',', 'arrlistaempaquedettmp');" /><?php else:?>&nbsp;&nbsp;&nbsp;<b>X</b><?php endif;?></td>
