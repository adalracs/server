<?php 
	...
	for( $i=0; $i < $nr_medici; $i++ )
	{
		...
		$row = fncfetch($rs_medici, $i);
	  	$data[] = (int) $row[medicicantid] - $row[medicicantid];
	  	$tagsname[] = $row[medicifecmed];
		} else {
		$row = fncfetch($rs_medici, $i);
		$row2 = fncfetch($rs_medici, $i-1);
		$data[] = (int) $row[medicicantid] - $row2[medicicantid];
	  	$tagsname[] = $row[medicifecmed];
		}
	  	if($cont < (int) $row[medicicantid])
	  		$cont = (int) $row[medicicantid] - $row2[medicicantid];
	} 
